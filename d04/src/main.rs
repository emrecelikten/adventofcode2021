...
use common::file_io::read_file_as_string_groups;
use std::str::FromStr;
...
fn parse_input<S: AsRef<str>>(chunks: &[Vec<S>]) -> (Vec<u64>, Vec<Board>) {
    let draws = chunks[0][0]
        .as_ref()
        .split(",")
        .map(|draw| u64::from_str(draw).unwrap())
        .collect();
...
fn main() -> Result<(), CommonError> {
    let chunks = read_file_as_string_groups("input")?;
...
mod tests {
    use super::*;
    use common::file_io::split_double_newline;
...
22 11 13  6  5
 2  0 12  3  7";
    ...
    fn test_check_victory() {
        ...
        assert_eq!(board_loss.check_victory(), false);

        let mut board_row_victory = board_loss.clone();
        board_row_victory.marked[2] = [true; 5];
        assert_eq!(board_row_victory.check_victory(), true);

        let mut board_col_victory = board_loss.clone();
        for row in 0..5 {
            board_col_victory.marked[row][2] = true;
        }
        assert_eq!(board_col_victory.check_victory(), true);
    }

    #[test]
    fn test_play() {
        let chunks = split_double_newline(&TEST_DATA);
        let (draws, boards) = parse_input(&chunks);
