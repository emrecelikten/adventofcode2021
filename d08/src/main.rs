...
use common::file_io::read_file_as_structs;
...
fn find_map(disp: &Disp) -> Option<BTreeMap<char, char>> {
    ...
    common::search::dfs(initial_state, |x| check_solution(x), expand_states)
...
fn main() -> Result<(), CommonError> {
    let disp_vec: Vec<Disp> = read_file_as_structs("input")?;
...
mod tests {
    ...
    const TEST_DATA: &'static str = r"be cfbegad cbdgef fgaecd cgeb fdcge agebfd fecdb fabcd edb | fdgacbe cefdb cefbgd gcbe
...
gcafb gcf dcaebfg ecagb gf abcdeg gaef cafbge fdbac fegbdc | fgae cfgab fg bagce";
    ...
    fn test_count_unique_outputs() {
        let lines = transform_iter(TEST_DATA.lines(), |e| Disp::from_str(e)).unwrap();
    ...
    #[test]
    fn test_find_map() {
        let lines = transform_iter(std::iter::once("acedgfb cdfbe gcdfa fbcad dab cefabd cdfgeb eafb cagedb ab | cdfeb fcadb cdfeb cdbaf"), |e| Disp::from_str(e)).unwrap();
