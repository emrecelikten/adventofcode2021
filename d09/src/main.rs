...
fn calculate_basin_size_mul(grid: &ArrayGrid<char>, minima_positions: &[CoordPair]) -> usize {
    ...
    for &(x, y) in minima_positions {
        let (num_filled, new_grid) = cur_grid.boundary_fill_cross(x, y, '*', '9');
        cur_grid = new_grid;
        if num_filled > 0 {
...
mod tests {
    ...
    const TEST_DATA: &'static str = r"2199943210
3987894921
9856789892
8767896789
9899965678";
    ...
    fn test_calculate_minima_sum() {
        let grid = ArrayGrid::from_str(&TEST_DATA).unwrap();
        let minima_positions = find_minima_positions(&grid);
